...
[DebuggerStepThrough]
public class RateLimitingValidatorDecorator<T> : IValidator<T> where T: class
{
    ...
    public RateLimitingValidatorDecorator(IValidator<T> decoratedValidator, ILogger logger, RateLimiterConstraints constraints)
    {
        ...
        _logger = logger?.ForContext(typeof(RateLimitingValidatorDecorator<T>)) ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<ValidationResult> ValidateAsync(T objectToValidate, CancellationToken cancellationToken = default)
    {
        ...
        if (!IsValidatorConfigured() || _rateLimiter == null) return await _decoratedValidator.ValidateAsync(objectToValidate, cancellationToken);
    ...
    private bool IsValidatorConfigured() => (_constraints.HasKey(_validatorType));
}